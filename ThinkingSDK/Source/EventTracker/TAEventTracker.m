...
#if TARGET_OS_IOS
            NSDictionary *dic = [[ThinkingAnalyticsSDK sharedInstanceWithAppid:self.config.appid].encryptManager encryptJSONObject:event];
#elif TARGET_OS_OSX
            NSDictionary *dic = event;
#endif
            count = [self.dataQueue addObject:dic withAppid:[self.config getMapInstanceToken]];
        } else {
